...
EXCEL_PATH = "HOTELBAR.xlsx"       # Path to your HOTELBAR Excel file
...
def main():
    ...
    # Start a single transaction for all rows (optional but recommended)
    cursor.execute("BEGIN TRANSACTION;")

    # 3) Loop over each row in the DataFrame
    for idx, row in df.iterrows():
        # Extract row data, ensuring we handle quotes
        persona = str(row.get('Persona trabajadora', '')).replace("'", "''")
        fecha_antig = str(row.get('Fecha antiguidad', '')).split(' ')[0]  # remove time portion if any
        ...
        # The newly created employee_id for foreign keys
        ...
        # We'll store DNI/bank in 'employment_status' just like the others
        insert_admin = f"""
        ...
        # We'll interpret "Fecha antigüidad" as date_joined
    ...
    conn.close()
    print("\nAll HOTELBAR rows have been inserted successfully!")
